...
app.post('/api/searchGroups', async (req, res, next) => 
{
   ...
   // incoming: userId, search
      ...
      const { field, search } = req.body;
         ...
         const db = client.db("StudyBuddy");
      const filter = await db.collection('filters').find({name:{$regex:field+'.*'}}).toArray();
      const results = await db.collection('groups').find({filter[0]:{$regex:search+'.*'}}).toArray(); 
/*
 ...
 */
      ...
      var ret2 = {filter:filter[0], results:ret, error:error};
